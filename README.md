...
1. **Ingestion (Veri Toplama):** Ağ trafiğini simüle eden C# .NET tabanlı `TestSender` yazılımı ile besleme yapılır. Testler üç senaryoda doğrulanmıştır:
...
2. **Parsing & Normalization:** Karmaşık metin yığınları Regex ve key-value eşleşmeleriyle yapılandırılmış nesnelere dönüştürülür.
3. **Persistence (Veri Saklama):** Yapılandırılmış veriler ve ham loglar NoSQL mimaride (**MongoDB**) arşivlenir.
4. **Correlation & Analysis:** CEF formatındaki `Severity` alanı 0-10 skalasında değerlendirilir:
...
5. **Visualization & Reporting:** Verileri anlamlı siber istihbarata dönüştüren **Streamlit** tabanlı görselleştirme katmanıdır.
